...
public class Simulation {
	...
	public static double Simulate(double[] weights, List<Resource> testResources, List<User> testUsers,
			int maxIterations) {
		HeuristicGenerator hg = new HeuristicGenerator(weights);
		PriorityQueue<Schedule> q = new PriorityQueue<Schedule>(
				(s1, s2) -> -Double.compare(s1.getScore() + hg.getHeuristic(s1), s2.getScore() + hg.getHeuristic(s2)));
