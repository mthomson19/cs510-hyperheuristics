...
public class Resource {
	...
	public double getRemainingSpace(Range r) {
		return (allocator.unallocatedSize - allocator.allocatedSpace.keySet().stream().filter(a -> r.overlaps(a))
				.mapToDouble(a -> a.size() * allocator.allocatedSpace.get(a)).sum()) / r.size();
	}
